...
class AssignedOrdersDataTable extends OrdersDataTable
{
    ...
    {
        return $model::with('academicLevel', 'citation', 'discipline', 'deadline', 'writer')
            ->select('orders.*')
            ->where(function ($query) {
                $query->where('orders.status', OrderStatus::ASSIGNED)
                    ->orWhere('orders.status', OrderStatus::REASSIGNMENT_REQUESTED)
                    ...
                    ->orWhere('orders.status', OrderStatus::ADMIN_REVIEW_REASSIGNMENT_REQUESTED)
                    ->orWhere('orders.status', OrderStatus::ADMIN_REVISION_DEADLINE_EXTENSION_REQUESTED)
                    ->orWhere('orders.status', OrderStatus::REVIEW_BY_ADMIN)
                    ->orWhere('orders.status', OrderStatus::SUBMITTED);
            });
    }
