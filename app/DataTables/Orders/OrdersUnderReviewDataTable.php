...
class OrdersUnderReviewDataTable extends OrdersDataTable
{
    /**
    ...
    public function query(Order $model)
    {
        return $model::with('academicLevel', 'citation', 'discipline', 'writer')
            ...
            ->where(function ($query) {
                $query->where('orders.status', OrderStatus::PENDING_CLIENT_REVIEW_CONFIRMATION)
                    ->orWhere('orders.status', OrderStatus::CLIENT_REVIEW_SUBMISSION)
                    ->orWhere('orders.status', OrderStatus::REVIEW_REASSIGNMENT_REQUESTED)
                    ->orWhere('orders.status', OrderStatus::REVISION_DEADLINE_EXTENSION_REQUESTED)
                    ->orWhere('orders.status', OrderStatus::REVIEW_BY_CLIENT);
            });
    }
    ...
    {
        return 'orders_under_review_'.date('YmdHis');
    }
}
